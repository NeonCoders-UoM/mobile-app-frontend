...
Your Flutter web app is getting a CORS error when trying to access your .NET backend API. This is a common issue when a web application running on one port (Flutter on `localhost:51443`) tries to access an API on another port (.NET on `localhost:5039`).
...
2. Start it again
3. Make sure it's running on `http://localhost:5039`
